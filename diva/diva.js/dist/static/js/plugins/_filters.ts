...
export function addFilterToQueue (data: CanvasImageData, filter: ((r: number, g: number, b: number) => number[]), adjust: number, name: string)
{
    ...
    if (index !== -1) // adjust a filter already in the queue
    {
        ...
        for (let i = index + 1, len = _filterQueue.length; i < len; i++)
        {
            ...
            if (otherFiltObj.name === 'Invert' && !inverted)
            {
                // don't reinvert the image
                continue;
            }
 ...
 **/
function _getOffscreenCanvasData (w: number, h: number): ImageData
{
...
function _manipulateImage (data: Uint8ClampedArray, func, adjustment)
{
    let len = data.length;
...
{
    let dataArr: Uint8ClampedArray = new Uint8ClampedArray(data.data);
 ...
 **/
export function grayscale(data: CanvasImageData)
{
...
export function saturation (data: CanvasImageData, adjust: number)
{
 ...
 * @returns {Array} - The computed RGB values for the input, with a constant 255 for the alpha channel.
 **/
function _saturation (r: number, g: number, b: number, adjust: number): number[]
{
    let adj = adjust * -0.01;
 ...
 * @returns {Array} - The computed RGB values for the input, with a constant 255 for the alpha channel.
 **/
function _vibrance (r: number, g: number, b: number, adjust: number): number[]
{
    let adj = adjust * -1;
...
export function brightness (data: CanvasImageData, adjust: number)
{
...
function convolve (data, weights: string | any[], opaque: undefined)
{
