...
def _compile_field_to_tuple(field, name, serializer_cls):
    ...
    return (name, getter, to_value, field.call, field.required, label,
            field.getter_takes_serializer)


class LDSerializerMeta(type):
    ...
    def _get_fields(direct_fields, serializer_cls):
        ...
        for cls in serializer_cls.__mro__[::-1]:
            if issubclass(cls, LDSerializerBase):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        real_cls = super(LDSerializerMeta, cls).__new__(cls, name, bases, attrs)
...
class LDSerializer(six.with_metaclass(LDSerializerMeta, LDSerializerBase)):
    ...
    def __init__(self, instance=None, many=False, data=None, context=None,
        ...
        super(LDSerializer, self).__init__(**kwargs)
