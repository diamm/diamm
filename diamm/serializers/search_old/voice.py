...
class VoiceSearchSerializer(serpy.Serializer):
    ...
    label_s = serpy.StrField(attr="label", required=False)

    position_s = serpy.StrField(attr="position", required=False)
    item_i = serpy.IntField(attr="item.id")
    ...
    voice_text_s = serpy.StrField(attr="voice_text")
