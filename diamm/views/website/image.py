...
def protected_image_auth(request: HttpRequest) -> HttpResponse:
    ...
    backend_uri = _resolve_backend_uri_from_request_path(original_uri)
    if not backend_uri:
        log.debug(
            "Protected image auth rejected: no backend URI for user id=%s original_uri=%s",
            user.pk,
            original_uri,
        )
        return HttpResponse(status=status.HTTP_403_FORBIDDEN)
    ...
    log.debug(
        "Protected image auth ok: user id=%s original_uri=%s backend_uri=%s",
        user.pk,
        original_uri,
        backend_uri,
    )
    response = HttpResponse(status=status.HTTP_204_NO_CONTENT)
    response["X-DIAMM-Backend-URI"] = backend_uri
    return response
...
def _resolve_backend_uri_from_request_path(request_path: str) -> str | None:
    ...
    return f"{settings.DIAMM_IMAGE_SERVER}{location}{suffix}"
