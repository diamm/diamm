...
from diamm.serializers.search_old.bibliography import BibliographySearchSerializer
...
from diamm.serializers.search_old.item_bibliography import (
    ItemBibliographySearchSerializer,
)
...
from diamm.serializers.search_old.voice import VoiceSearchSerializer
...
class Command(BaseCommand):
    def _index(self, objects, name_field, serializer):
        docs = []
        for i, obj in enumerate(objects.iterator(chunk_size=1000), 1):
            name = f"Object {obj.pk}" if not name_field else getattr(obj, name_field)

            self.stdout.write(
                f"{term.blue('Indexing')} {term.green(obj.__class__.__name__)}: {term.yellow(name)} ({term.yellow(str(obj.pk))})"
            )
    ...
    def _index_sources(self):
        ...
        objs = (
            ...
            .prefetch_related(
                "inventory__composition__composers__composer",
                "pages__images",
                "sets",
                "notations",
            )
    ...
    def _index_inventories(self):
        ...
        objs = (
            Item.objects.all()
            .order_by("pk")
            .select_related("source__archive__city__parent", "composition")
        )
    ...
    def _index_composers_inventory(self):
        ...
        objs = (
            Source.objects.all()
            .order_by("pk")
            .select_related("archive__city__parent")
            .iterator()
        )

        for source in objs:
            self.stdout.write(
                f"{term.blue('Indexing')} {term.green('Source (Composer Inventory)')}: {term.yellow(source.display_name)} ({term.yellow(str(source.pk))})"
            )
    ...
    def handle(self, *args, **kwargs):
        ...
        c = requests.get(
            "http://localhost:8983/solr/diamm_ingest/update/?commit=true", timeout=10
        )
        print(f"Done committing. Status code: {c.status_code}")
        print("Swapping ingest and live cores")
        r = requests.get(
            "http://localhost:8983/solr/admin/cores?action=SWAP&core=diamm&other=diamm_ingest",
            timeout=10,
        )
